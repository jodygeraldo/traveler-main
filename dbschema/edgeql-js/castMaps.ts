...
export type scalarAssignableBy<T extends $.ScalarType> = T extends _std.$number
  ? _std.$number
  : T extends _sys.$VersionStage
  ? _sys.$VersionStage
  : T extends _sys.$TransactionIsolation
  ...
  : T extends _std.$json
  ? _std.$json
  : T extends _std.$int64
  ? _std.$int64
  : T extends _std.$int32
  ? _std.$int32
  : T extends _std.$int16
  ? _std.$int16
  : T extends _std.$float64
  ? _std.$float64
  : T extends _std.$float32
  ? _std.$float32
  : T extends _std.$duration
  ? _std.$duration
  : T extends _std.$decimal
  ? _std.$decimalλIAssignableBy
  : T extends _std.$datetime
  ? _std.$datetime
  : T extends _std.$bytes
  ? _std.$bytes
  : T extends _std.$bool
  ? _std.$bool
  : T extends _std.$bigint
  ? _std.$bigint
  : T extends _schema.$Volatility
  ? _schema.$Volatility
  : T extends _schema.$TypeModifier
  ? _schema.$TypeModifier
  : T extends _schema.$TargetDeleteAction
  ? _schema.$TargetDeleteAction
  : T extends _schema.$ParameterKind
  ? _schema.$ParameterKind
  : T extends _schema.$OperatorKind
  ? _schema.$OperatorKind
  : T extends _schema.$Cardinality
  ? _schema.$Cardinality
  : T extends _default.$Weapon
  ? _default.$Weapon
  : T extends _default.$Vision
  ? _default.$Vision
  : T extends _default.$Region
  ? _default.$Region
  : T extends _cfg.$memory
  ? _cfg.$memory
  : T extends _cfg.$AllowBareDDL
  ? _cfg.$AllowBareDDL
  : T extends _cal.$relative_duration
  ? _cal.$relative_duration
  : T extends _cal.$local_time
  ? _cal.$local_time
  : T extends _cal.$local_datetime
  ? _cal.$local_datetime
  : T extends _cal.$local_date
  ? _cal.$local_date
  : never

export type scalarCastableFrom<T extends $.ScalarType> = T extends _std.$number
  ? _std.$number
  : T extends _sys.$VersionStage
  ? _sys.$VersionStage
  : T extends _sys.$TransactionIsolation
...
type getSharedParentScalar<A, B> = A extends _std.$number
  ...
  ? B extends _schema.$Cardinality
    ? B
    : never
  : A extends _default.$Weapon
  ? B extends _default.$Weapon
    ? B
    : never
  : A extends _default.$Vision
  ? B extends _default.$Vision
    ? B
    : never
  : A extends _default.$Region
  ? B extends _default.$Region
    ? B
    : never
...
function getSharedParentScalar<A extends $.ScalarType, B extends $.ScalarType>(
  ...
  if (a.__name__ === 'default::Weapon') {
    if (b.__name__ === 'default::Weapon') {
      return b
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`)
  }
  if (a.__name__ === 'default::Vision') {
    if (b.__name__ === 'default::Vision') {
      return b
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`)
  }
  if (a.__name__ === 'default::Region') {
    if (b.__name__ === 'default::Region') {
      return b
    }
    throw new Error(`Types are not castable: ${a.__name__}, ${b.__name__}`)
  }
...
export type scalarWithConstType<
  ...
  TsConstType
> = $.ScalarType<T['__name__'], T['__tstype__'], T['__castable__'], TsConstType>
export type literalToScalarType<T extends any> = T extends number
  ...
  : T extends edgedb.Duration
  ? scalarWithConstType<_std.$duration, T>
  : T extends edgedb.LocalDateTime
  ? scalarWithConstType<_cal.$local_datetime, T>
  : T extends edgedb.LocalDate
  ? scalarWithConstType<_cal.$local_date, T>
  : T extends edgedb.LocalTime
  ? scalarWithConstType<_cal.$local_time, T>
  : T extends edgedb.RelativeDuration
  ? scalarWithConstType<_cal.$relative_duration, T>
  : T extends edgedb.ConfigMemory
